"""
劳动法智能助手 v3（合并版）
= v2 全部功能（三 Tab + SQLite 持久化历史）
+ law_app 的流式输出 + 多轮对话上下文
运行：py -m streamlit run law_app_v3.py
"""
import os
...
with tab1:
    ...
    def answer(question):
        ...
        # 多轮上下文：把当前问题之前的历史拼成"对话背景"送进 RAG
        # 这样追问"那节假日呢"时，RAG 能知道上文在聊加班费
        prior = st.session_state.messages[:-1]  # 去掉刚加进去的当前问题
        if prior:
            ctx_lines = []
            for m in prior[-6:]:
                role_label = "用户" if m["role"] == "user" else "助手"
                ctx_lines.append(f"{role_label}：{m['content'][:200]}")
            rag_query = "【对话背景】\n" + "\n".join(ctx_lines) + f"\n\n【当前问题】\n{question}"
        else:
            rag_query = question
            ...
            with st.spinner("正在查阅法规并综合分析..."):
                try:
                    resp = loop.run_until_complete(
                        rag.aquery(rag_query, param=QueryParam(mode="hybrid"))
                    )
                except Exception as e:
                    resp = f"抱歉，查询出错了：{e}\n\n请稍后重试。"

            # 流式输出：spinner 结束后逐字打出，提升体感
            def _stream():
                for char in resp:
                    yield char
                    time.sleep(0.01)
...
{text_chunk}"""
                    ...
                    note = citation_check.format_note(analysis)
                    if note:
                        st.caption(note)
